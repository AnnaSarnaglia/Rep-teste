...
{ // Edsger Dijkstra, Partido do Melhor Caminho (PMC), P, 10
  // Tim Berners-Lee, World Wide Web (WWW), G, 36
    tCandidato c;

    strcpy(c.nome, nome);
    strcpy(c.partido, partido);
    c.cargo = cargo; // letra só
    c.id = id;
    // c.votos = 0;
    return c;
}
...
{ // Carol Shaw, Mulheres Programadoras Unidas (MPU), P, 26
    // Linus Torvalds, Partido: Linux (Linux), G, 64
    char nome[50], partido[50], cargo;
    int id, voto;
    scanf(" %[^,],", nome); // le até a virgula
    scanf(" %[^,],", partido);
    scanf(" %c", &cargo); // letra só
    scanf(" %d", &id);
    voto = 0;

    // ImprimeCandidato(c, 0);
...
{
    float votos = candidato.votos, totalv = totalVotos;
    if (totalv != 0)
    {
